...
async function evaluateCategory(tmId, cat, otherCat) {
    if (cat.type === 'goals_club_50' || cat.type === 'assists_club_50') {
        ...
        return new Promise(resolve => {
            db.get(`SELECT 1 FROM ${table} WHERE tm_id = ? AND (club_name LIKE ? OR club_name LIKE ?) AND ${col} >= 50 LIMIT 1`,
...
function getCandidateTmIds(cat, otherCat) {
    return new Promise((resolve) => {
        if (cat.type === 'goals_club_50' || cat.type === 'assists_club_50') {
            ...
            const col = cat.type === 'goals_club_50' ? 'goals' : 'assists';
            db.all(
                `SELECT DISTINCT tm_id FROM ${table} WHERE (club_name LIKE ? OR club_name LIKE ?) AND ${col} >= 50 LIMIT 500`,
        ...
        switch (cat.type) {
            ...
            case 'league': sql = "SELECT DISTINCT tm_id FROM player_leagues WHERE (league_code LIKE ? OR league_code LIKE ?) LIMIT 500"; params = [`%${LEAGUE_MAP[cat.value]}%`, `%${LEAGUE_MAP[cat.value].replace(' ', '')}%`]; break;
