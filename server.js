...
// IMPORTANT: always anchor the DB next to this file.
// `path.resolve(__dirname, ...)` can still be surprising if `__dirname` is not what you expect in some launch modes;
// `path.join(__dirname, ...)` is the usual stable pattern. Override with DB_PATH if needed.
const dbPath = process.env.DB_PATH
    ? path.resolve(process.env.DB_PATH)
    : path.join(__dirname, 'schweizer_fussball_grid.db');
...
db.run(`CREATE TABLE IF NOT EXISTS player_club_season_goals (tm_id INTEGER, club_name TEXT, season_name TEXT, goals INTEGER, PRIMARY KEY(tm_id, club_name, season_name))`);
db.run(`CREATE TABLE IF NOT EXISTS player_club_season_assists (tm_id INTEGER, club_name TEXT, season_name TEXT, assists INTEGER, PRIMARY KEY(tm_id, club_name, season_name))`);
...
/** How many past daily grids (by date) inform club usage weighting. */
const GRID_HISTORY_CLUB_LOOKBACK = 30;
...
function weightedClubSelection(availableClubs, count, usageCounts) {
    if (!count || !availableClubs.length) return [];
    ...
    for (let i = 0; i < count && pool.length > 0; i++) {
        const sumW = poolWeights.reduce((a, b) => a + b, 0);
        if (sumW <= 0) break;
...
/** Past grids (newest first), excluding `beforeDateIso`, for club usage. */
function loadRecentGridsForClubUsage(beforeDateIso, limit, cb) {
    db.all(
        'SELECT grid_data FROM daily_grids WHERE date < ? ORDER BY date DESC LIMIT ?',
        [beforeDateIso, limit],
        (err, rows) => {
            ...
            for (const row of rows || []) {
                try {
                    grids.push(JSON.parse(row.grid_data));
                } catch (_) {
                    /* skip corrupt row */
                }
            }
...
function generateDailyGridData(dateStr, clubUsageCounts) {
    const counts = clubUsageCounts || getClubUsageCounts([]);
    ...
    const specials = [
        ...
        { type: 'red_card_club_1', value: 'RC1', label: '≥ 1 Rote Karte für Club' },
        { type: 'yellow_cards_club_20', value: 'YC20', label: '> 20 Gelbe Karten für Club' },
    ...
    if (allowChallengeToday && challengeClubs.length > 0) {
        const pickedChallenge = weightedClubSelection(challengeClubs, 1, counts);
        const needRegular = Math.max(0, numClubsNeeded - pickedChallenge.length);
        const regularPicks = weightedClubSelection(regularClubs, needRegular, counts);
        selectedClubs = shuffleWithRng([...pickedChallenge, ...regularPicks], rng).slice(0, numClubsNeeded);
    } else {
        selectedClubs = shuffleWithRng(
            weightedClubSelection(regularClubs, numClubsNeeded, counts),
            rng
        );
    }
...
app.get('/api/daily-grid', async (req, res) => {
    ...
    db.get("SELECT grid_data FROM daily_grids WHERE date = ?", [today], async (err, row) => {
        ...
        loadRecentGridsForClubUsage(today, GRID_HISTORY_CLUB_LOOKBACK, (histErr, recentGrids) => {
            if (histErr) return res.status(500).json({ error: 'Grid history failed' });
            let gridData;
            try {
                const usageCounts = getClubUsageCounts(recentGrids);
                gridData = generateDailyGridData(today, usageCounts);
            } catch (e) {
                return res.status(500).json({ error: 'Grid generation failed' });
            }
...
function getClubStatValue(tmId, clubKey, table, col) {
    return new Promise((resolve) => {
        const patterns = [`%${clubKey}%`];
        if (CLUB_MAP[clubKey]) patterns.push(`%${CLUB_MAP[clubKey]}%`);
        const placeholders = patterns.map(() => 'club_name LIKE ?').join(' OR ');
        db.get(
            `SELECT MAX(${col}) AS v FROM ${table} WHERE tm_id = ? AND (${placeholders})`,
            [tmId, ...patterns],
            (err, row) => resolve(row && row.v != null ? Number(row.v) : 0)
        );
    });
}

function getSeasonStatValue(tmId, table, col) {
    return new Promise((resolve) => {
        db.get(
            `SELECT MAX(${col}) AS v FROM ${table} WHERE tm_id = ?`,
            [tmId],
            (err, row) => resolve(row && row.v != null ? Number(row.v) : 0)
        );
    });
...
async function getThresholdClosenessBonus(player, rowCat, colCat) {
    // Goal: for threshold categories (>=X), "just above X" should score higher
    // than "way above X". Bonus decays quickly with how far above the threshold
    // the player is. Returns 0 if not applicable.
    async function bonusForCat(cat, otherCat) {
        if (!cat) return 0;

        // Club totals thresholds (requires team in the other dimension)
        if ((cat.type === 'goals_club_50' || cat.type === 'assists_club_50') && otherCat && otherCat.type === 'team') {
            const isGoals = cat.type === 'goals_club_50';
            const minValue = isGoals ? 25 : 15;
            const table = isGoals ? 'player_club_goals' : 'player_club_assists';
            const col = isGoals ? 'goals' : 'assists';
            const v = await getClubStatValue(player.tm_id, otherCat.value, table, col);
            const delta = Math.max(0, v - minValue);
            // 0 above threshold -> +2.0, 1 -> ~+1.8, 10 -> ~+0.7, 50 -> ~+0.1, 100+ -> ~0
            return 2.0 * Math.exp(-delta / 10);
        }

        // Season thresholds (team-specific if otherCat is team, else overall season)
        if (cat.type === 'goals_season_10' || cat.type === 'assists_season_10') {
            const isGoals = cat.type === 'goals_season_10';
            const minValue = 10;
            let v = 0;
            if (otherCat && otherCat.type === 'team') {
                const table = isGoals ? 'player_club_season_goals' : 'player_club_season_assists';
                const col = isGoals ? 'goals' : 'assists';
                v = await getClubStatValue(player.tm_id, otherCat.value, table, col);
            } else {
                const table = isGoals ? 'player_season_goals' : 'player_season_assists';
                const col = isGoals ? 'goals' : 'assists';
                v = await getSeasonStatValue(player.tm_id, table, col);
            }
            const delta = Math.max(0, v - minValue);
            // Smaller bonus for season thresholds
            return 1.0 * Math.exp(-delta / 6);
        }

        return 0;
    }

    // Consider both dimensions; if both are threshold categories, the player must satisfy both,
    // and "tightness" on either should be rewarded.
    const [bRow, bCol] = await Promise.all([
        bonusForCat(rowCat, colCat),
        bonusForCat(colCat, rowCat),
    ]);
    return bRow + bCol;
}
...
async function calculateRarity(player, rowCat, colCat) {
    ...
    // Threshold tightness bonus: for ">= X" categories, just-over-the-line should be worth more.
    const tightnessBonus = await getThresholdClosenessBonus(player, rowCat, colCat);

    let rarity = baseScore + vintageBonus + obscurityBonus + tightnessBonus;
...
async function evaluateCategory(tmId, cat, otherCat) {
    if (cat.type === 'red_card_club_1') {
        if (!otherCat || otherCat.type !== 'team') return false;
        return new Promise(resolve => {
            db.get(
                `SELECT 1 FROM player_club_red_cards WHERE tm_id = ? AND (club_name LIKE ? OR club_name LIKE ?) AND red_cards >= 1 LIMIT 1`,
                [tmId, `%${CLUB_MAP[otherCat.value]}%`, `%${otherCat.value}%`],
                (e, r) => resolve(!!r)
            );
        });
    }
    if (cat.type === 'yellow_cards_club_20') {
        if (!otherCat || otherCat.type !== 'team') return false;
        return new Promise(resolve => {
            db.get(
                `SELECT 1 FROM player_club_yellow_cards WHERE tm_id = ? AND (club_name LIKE ? OR club_name LIKE ?) AND yellow_cards > 20 LIMIT 1`,
                [tmId, `%${CLUB_MAP[otherCat.value]}%`, `%${otherCat.value}%`],
                (e, r) => resolve(!!r)
            );
        });
    ...
    if (cat.type === 'goals_season_10' && otherCat && otherCat.type === 'team') {
        return new Promise(resolve => {
            db.get(
                `SELECT 1 FROM player_club_season_goals WHERE tm_id = ? AND (club_name LIKE ? OR club_name LIKE ?) AND goals >= 10 LIMIT 1`,
                [tmId, `%${CLUB_MAP[otherCat.value]}%`, `%${otherCat.value}%`],
                (e, r) => resolve(!!r)
            );
        });
...
function getCandidateTmIds(cat, otherCat) {
    return new Promise((resolve) => {
        if (cat.type === 'red_card_club_1') {
            ...
            db.all(
                `SELECT DISTINCT tm_id FROM player_club_red_cards WHERE (club_name LIKE ? OR club_name LIKE ?) AND red_cards >= 1 LIMIT 500`,
        ...
        if (cat.type === 'yellow_cards_club_20') {
            if (!otherCat || otherCat.type !== 'team') return resolve([]);
            db.all(
                `SELECT DISTINCT tm_id FROM player_club_yellow_cards WHERE (club_name LIKE ? OR club_name LIKE ?) AND yellow_cards > 20 LIMIT 500`,
                [`%${CLUB_MAP[otherCat.value]}%`, `%${otherCat.value}%`],
                (e, rows) => resolve((rows || []).map((r) => r.tm_id))
            );
            return;
        ...
        if (cat.type === 'goals_season_10' && otherCat && otherCat.type === 'team') {
            db.all(
                `SELECT DISTINCT tm_id FROM player_club_season_goals WHERE (club_name LIKE ? OR club_name LIKE ?) AND goals >= 10 LIMIT 500`,
                [`%${CLUB_MAP[otherCat.value]}%`, `%${otherCat.value}%`],
                (e, rows) => resolve((rows || []).map((r) => r.tm_id))
            );
            return;
